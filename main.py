...
class App(ttk.Window):
    ...
    def draw_menu(self):
        ...
        menu.add_cascade(label='Change theme', menu=settings_menu)
    ...
    def save_logs(self):
        ...
        with open(filepath, 'w') as f:
            ...
            for i in range(len(data[0])):
                f.write(f"Beacon {i+1} coordinates: {data[0][i]}, ri={data[3][i]}\n")
            f.write(f"Receiver coordinates: {data[1]}, tau={data[2]}\n")
...
class LogFrame(ttk.Frame):
    def __init__(self, parent):
        ...
        self.rowconfigure(0, weight=1)
        self.columnconfigure(0, weight=1)
        self.log_window = ttk.Label(self, justify=ttk.LEFT, relief=ttk.GROOVE,
...
# Frame with simulation parameters
class ParametersFrame(ttk.Frame):
    def __init__(self, parent):
        ...
        for i in range(1, 4):
            self.create_beacon_entry(i, i).grid(row=i-1, column=0)
    ...
    def add_beacon(self):
        """
        Create entries for new beacon

        :return:
        """
        beacons = self.grid_size()[1]
        self.rowconfigure(beacons, weight=1)
        self.create_beacon_entry(beacons+1, beacons+1).grid(row=beacons, column=0)

    def create_beacon_entry(self, beacon_i: int, r_i: str) -> ttk.Frame:
        """
        ...
        """
        beacon_frame = ttk.Frame(self, name=f'beacon_frame{beacon_i}')
        self.columnconfigure((0, 1, 2), weight=1)
        self.rowconfigure((0, 1), weight=1)

        beacon_label = ttk.Label(beacon_frame, text=f"beacon {beacon_i} coordinates")
        ri_label = ttk.Label(beacon_frame, text=f'r{r_i}')
    ...
    def create_epoch_entry(self):
        """
        Create frame with entries for epochs
    ...
    def show_warnings(self, warning_type: str):
